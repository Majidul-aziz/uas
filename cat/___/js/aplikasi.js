...
function m_soal_s() {
	...
	$.ajaxFileUpload({
		...
		success: function (data) {
			...
			if (d.status == 'ok') {
				Swal.fire('Sukses', 'Berhasil Tambah Soal', 'success')
					.then((result) => {
						if (result.value) {
							window.location.href = base_url + "adm/m_soal/pilih_mapel/" + d.id_mapel;
						}
					});
...
function m_ujian_s() {
	...
	if (form.jumlah_soal > form.jumlah_soal1) {
		...
		$.ajax({
			...
			if (response.status == "ok") {
				Swal.fire('Sukses', 'Berhasil Tambah Ujian', 'success')
					.then((result) => {
						if (result.value) {
							window.location.href = base_url + "adm/m_ujian";
						}
					});
			} else {
				console.log('gagal');
...
function m_siswa_matkul_s() {
	...
	$.ajax({
		...
		if (response.status == "ok") {
			Swal.fire('Sukses', 'Berhasil Pilih Matakuliah', 'success')
				.then((result) => {
					if (result.value) {
						window.location.href = base_url + "adm/m_siswa";
					}
...
function m_guru_s() {
	...
	$.ajax({
		...
		if (response.status == "ok") {
			Swal.fire('Sukses', 'Berhasil Tambah Data', 'success')
				.then((result) => {
					if (result.value) {
...
function m_guru_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			...
			success: function (response) {
				if (response.status == "ok") {
					Swal.fire('Sukses', 'Berhasil Hapus Data', 'warning')
						.then((result) => {
							if (result.value) {
								window.location.href = base_url + "adm/m_guru";
							}
...
function m_guru_u(id) {
	if (confirm('Anda yakin..? Username otomatis adalah "guru' + id + '", dan Password otomatis adalah "admin"')) {
		$.ajax({
			...
			success: function (response) {
				if (response.status == "ok") {
					Swal.fire('Sukses', 'User Telah Aktif', 'success')
						.then((result) => {
							if (result.value) {
								window.location.href = base_url + "adm/m_guru";
							}
...
function m_mapel_s() {
	...
	$.ajax({
		...
		if (response.status == "ok") {
			Swal.fire('Sukses', 'Berhasil Tambah Matakuliah', 'success')
				.then((result) => {
					if (result.value) {
...
function m_mapel_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			...
			success: function (response) {
				if (response.status == "ok") {
					Swal.fire('Sukses', 'Berhasil Hapus Matakuliah', 'success')
						.then((result) => {
							if (result.value) {
								window.location.href = base_url + "adm/m_mapel";
							}
